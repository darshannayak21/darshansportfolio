...
function GalleryScene({
	...
	useEffect(() => {
		...
		window.addEventListener('gallery-toggle', handleToggle as EventListener);

		const handleScroll = () => {
			const currentScrollY = window.scrollY;
			const deltaY = currentScrollY - lastScrollY;
			
			if (isActive && Math.abs(deltaY) > 0) {
				// Reduced speed multiplier from 0.05 to 0.01 for much slower rotation
		...
		return () => {
			window.removeEventListener('gallery-toggle', handleToggle as EventListener);
		...
		<>
			{planesData.current.map((plane, i) => {
				...
				const scale: [number, number, number] =
					aspect > 1 ? [2 * aspect * 1.8, 2 * 1.8, 1] : [2 * 1.8, (2 / aspect) * 1.8, 1];
